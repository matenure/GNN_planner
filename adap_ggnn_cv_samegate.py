...
from utils import glorot_init, SMALL_NUMBER, one_hot
...
class asSparseGGNNChemModel(ChemModel):
    ...
    def process_raw_graphs(self, raw_data: Sequence[Any], is_training_data: bool, context_file=None, batch_size=None) -> Any:
        processed_graphs = []
        for d in raw_data:
            (adjacency_lists, num_incoming_edge_per_type) = self.__graph_to_adjacency_lists(d['graph'], min(len(d["node_features"]), batch_size))
        ...
        if context_file is None:
            for ex_id in range(len(processed_graphs)):#generating new training data
                ...
                for pre_id in preplanner_ids:
                    ...
                    pair_graphs.append({"adjacency_lists": processed_graphs[ex_id]["adjacency_lists"],
                                        "num_incoming_edge_per_type": processed_graphs[ex_id]["num_incoming_edge_per_type"],
    ...
    def __graph_to_adjacency_lists(self, graph, num_nodes: int) -> Tuple[Dict[int, np.ndarray], Dict[int, Dict[int, int]]]:
        adj_lists = defaultdict(list)
        num_incoming_edges_dicts_per_type = defaultdict(lambda: defaultdict(lambda: 0))
        for src, e, dest in graph:
            if src >= num_nodes or dest >= num_nodes:
                continue
...
def main():
    args = docopt(__doc__)
    try:
        model = asSparseGGNNChemModel(args)
        ...
        if '--split_index' in args and args['--split_index'] is not None:
            ...
            contextfile = os.path.join(save_dir, 'planner1_ggnn_samegate_' + data_type + '_' + str(split_index) + '.npz')
            timeout, _, preds, skipped_graphs = model.pred(test_file, contextfile=contextfile)
            savefile = os.path.join(save_dir, 'planner2_ggnn_samegate_' + data_type + '_' + str(split_index) + '.npz')
            ...
            for i in range(10):
                model.train(i)
                # model.restore_model("models/lifted/2018-07-29-01-12-46_4493_asSparseGGNNChemModel_model_lifted.pickle")
                contextfile = os.path.join(save_dir, 'planner1_ggnn_samegate_' + data_type + '_' + str(i) + '.npz')
                if not os.path.exists(contextfile):
                    continue
                timeout, _, preds, skipped_graphs = model.pred(test_file, contextfile=contextfile)
                savefile = os.path.join(save_dir, 'planner2_ggnn_samegate_' + data_type + '_' + str(i) + '.npz')
