...
class SparseGGNNChemModel(ChemModel):
    ...
    def make_minibatch_iterator(self, data: Any, is_training: bool):
        ...
        while num_graphs < len(data):
            ...
            if len(batch_graph_nodes_list) == 0:
                continue
            batch_feed_dict = {
...
def main():
    ...
    try:
        ...
        if '--split_index' in args and args['--split_index'] is not None:
            ...
            savefile = os.path.join(save_dir, 'planner1_ggnn_samegate_' + data_type + '_' + str(split_index) + '.npz')
            np.save(open(savefile, "wb"), preds)
            print("Split: ", split_index, "Timeout number,", timeout)
        else:
            for i in range(10):
                model.train(i)
                # model.restore_model("./models/grounded/2018-08-08-22-52-24_4849_SparseGGNNChemModel_model_ground_samegate.pickle")
                preds, timeout, total = model.pred(test_file)
                savefile = os.path.join(save_dir, 'planner1_ggnn_samegate_'+data_type + '_'+str(i) + '.npz')
                np.save(open(savefile,"wb"), preds)
